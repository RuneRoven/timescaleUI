# Stage 1: Build Tailwind CSS (platform-independent)
...
# Stage 2: Build Go binary (native cross-compilation)
...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -ldflags="-s -w" -o /tsui ./cmd/tsui
